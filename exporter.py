...
logging_config = dict(
    version=1,
    formatters={
        'f': {'format': '%(asctime)s %(levelname)-8s [%(funcName)s:%(lineno)d] %(message)s'}
    },
    handlers={
...
class CustomCollector(object):
    ...
    def collect(self):
        ...
        packets = CounterMetricFamily(
            'tplink_sg_switch_port_packet_counters',
            'Packet counters (rx/tx, good/bad) for each switch port',
            labels=['host', 'port', 'type'],
            unit='packets'
        )
        linkSpeed = StateSetMetricFamily(
            'tplink_sg_switch_port_linkSpeed',
            'Link speed/duplex for each switch port',
            labels=['host', 'port']
        )
        linkState = StateSetMetricFamily(
            'tplink_sg_switch_port_linkState',
            'Administrative state (enabled/disabled) for each switch port',
            labels=['host', 'port']
        )
        portDescription = InfoMetricFamily(
            'tplink_sg_switch_port_description',
            'Port descriptions for each port',
            labels=['host', 'port']
        )

        # Fetch data for each switch and create metrics
        for current_switch in self.switches:
            ...
            if current_switch.loggedIn():
                ...
                for port, data in stats.items():
                    # Add packet counters for each port and packet type
                    for pktType in ('rxGoodPkt', 'rxBadPkt', 'txGoodPkt', 'txBadPkt'):
                        label = [current_switch.getIP(), port, pktType]
                        packets.add_metric(label, int(data[pktType]))

                    # Add link speed and link state for each port
                    linkSpeedValue = current_switch.getPortStateMapping(data['link_status'])
                    if linkSpeedValue is None:
                        # If link status is unknown, set a default value (e.g., 'unknown')
                        linkSpeedValue = 'unknown'
                    linkSpeed.add_metric(
                        labels=(current_switch.getIP(), port),
                        value={linkSpeedValue: True}
                    )
                    linkState.add_metric(
                        labels=(current_switch.getIP(), port),
                        value={'enabled': data['state'] == '1', 'disabled': data['state'] == '0'}
                    )

                    # Add port descriptions
                    portDescription.add_metric(
                        labels=(current_switch.getIP(), port),
                        value={'description': data['description']}
                    )

        # Yield the metrics for all switches
        yield packets
        yield linkSpeed
        yield linkState
...
if __name__ == '__main__':
    ...
    logger.info("Starting to serve metrics on port {}".format(http_port))
    try:
        while True:
            time.sleep(1)
